...
import base64
from project.ITH.image_denoising import denoising_config
from project.ITH.image_denoising import denoising_model
from project.ITH.image_classification import classification_config
from project.ITH.image_classification import classification_model
from project.ITH.image_similarity import similarity_config  # 配置文件
from project.ITH.image_similarity import similarity_model  # 自定义模型模块
...
app = Flask(__name__, static_folder='project/ITH/common/dataset')
...
encoder.load_state_dict(
    torch.load(
        os.path.join(
            '..',
            similarity_config.PROJECT_PACKAGE_NAME, similarity_config.ENCODE_MODEL_NAME),
        map_location=device))
...
embedding = np.load(os.path.join(
    '..',
    similarity_config.PROJECT_PACKAGE_NAME,
